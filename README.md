...
Ubiquiti hardware consoles (UDM family, UCG-Max / UCG-Ultra / UCG-Fiber, Cloud Key Gen2 Plus) don't have this problem. The mobile app direct-connects to them on-LAN, or across a site-to-site VPN, without touching the cloud. The self-hosted UOS Server behaves differently.
...
Without a DNS answer, the phone has no IP to connect to, and the app skips straight to cloud relay. Even if DNS did resolve, there's a port mismatch waiting: the mobile app probes port 443, but UOS Server serves its Network app on 11443 by default.

## The fix

Two pieces: a DNS override so the phone can find your controller, and a reverse proxy so port 443 reaches the UOS Server's 11443. If your UOS Server is already running on port 443 (dedicated host, nothing else bound to 443), step 2 is optional.
...
`tls: {}` uses Traefik's default self-signed cert. Let's Encrypt can't issue a cert for the `.ui.direct` zone (you don't own it), and as it turns out you don't need one - the iOS app accepted Traefik's self-signed cert without any cert install on the phone.
...
`lan-restricted` is what keeps this LAN-only. The hostname you're overriding is publicly resolvable - anyone who knows your console ID can look it up. Without an IP allowlist, someone could point their own DNS at your reverse proxy and probe it from the internet. Limit it to LAN source ranges and you're covered.
...
Pointing DNS directly at the UOS Server's LAN IP with no reverse proxy, if UOS is running on its default port 11443. The app hits port 443 and nothing answers.
